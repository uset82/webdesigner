...
import { WebDesignerOrchestrator } from '../core/orchestrator';

const orchestrator = new WebDesignerOrchestrator(process.cwd());
...
function loadOrCreateManifest(): any {
  ...
  try {
    switch (name) {
      ...
      case 'wd_add_artifact': {
        ...
        const emitted = args?.emitted as any[];

...
- Verify image-to-Three.js requests on Next.js or React/Vite add `img2threejs` to `integrations`.
- Verify image-only, static UI, Flutter, and backend-only requests do not add `img2threejs` automatically.
...
- `scenarios/img2threejs.intent.json` should resolve to `scenarios/img2threejs.expected-selection.json` and include the conditional `img2threejs` integration.

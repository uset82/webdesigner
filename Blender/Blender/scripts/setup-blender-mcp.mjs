...
export async function setupBlenderMcp(options = {}) {
  ...
  if (!addonIsEnabled(blenderPath)) {
    throw new Error("Pinned Blender MCP add-on is installed but not enabled. Run pnpm setup:blender-mcp.");
  }
...
async function main() {
  ...
  if (options.help) {
    console.log("Usage: node scripts/setup-blender-mcp.mjs [--blender <path>] [--verify-only] [--replace]");
    return;
  }

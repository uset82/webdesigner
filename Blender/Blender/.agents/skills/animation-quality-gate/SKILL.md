...
## Mascot / Cholita profile

For front-facing 2D Cholita / layered mascot motion (and Blender previews of the same character), use `--profile mascot`:

```bash
python .agents/skills/animation-quality-gate/scripts/animation_contact_sheet.py \
  --frames frame_01.png frame_02.png frame_03.png \
  --out-image .codex-avatar/previews/contact-sheet.png \
  --out-report .codex-avatar/previews/contact-sheet.json \
  --profile mascot \
  --fail-on-warnings
```

Mascot defaults are stricter than generic:

- max mean RGB frame jump `38`
- min silhouette IoU `0.82`
- max subject-center drift `0.12` of frame size

Also verify authored layer separation before accepting motion:

- hat / hair / face / eyes / mouth / scarf / cape / skirt / hands / feet / reactions remain independently controllable
- validate SVG with `validateSvgLayers(svg, { profile: "mascot" })`
- do not treat bitmap traces as animation-ready character layers
...
- Mascot passes must keep silhouette IoU and center drift inside profile thresholds, or document an intentional morph.

## Scripts

- `scripts/animation_contact_sheet.py` builds a contact sheet plus frame-diff, silhouette IoU, and subject-dominance report.
